...
export function UserNav() {
  ...
  if (isSignedIn) {
    ...
    const userInitials = `${user?.firstName?.[0] ?? ''}${user?.lastName?.[0] ?? ''}` || user?.emailAddresses[0]?.emailAddress[0] || 'U';
    const userFullName = `${user?.firstName ?? ''} ${user?.lastName ?? ''}`.trim() || user?.emailAddresses[0]?.emailAddress;
        ...
        <DropdownMenuTrigger asChild>
          ...
          <Button variant="ghost" className="relative h-10 w-10 rounded-full">
            <Avatar className="h-10 w-10">
              <AvatarImage src={user?.imageUrl} alt={userFullName || "User avatar"} />
